...
1. Read official docs: `https://auth.estifanos.com/getting-started/installation/` (mirrored at `https://convex-auth.pages.dev/getting-started/installation/`)
...
5. Check the latest tag on `https://github.com/robelest/convex-auth/releases` and record the version used.
6. Read current self hosting docs if portal or static hosting is involved:
...
Latest tagged release as of this skill revision is `v0.0.4-preview.28` (Apr 16, 2026). As of the `0.0.4-preview.25`-`0.0.4-preview.28` range the installed package ships:
...
As of `preview.28`, the upstream README lists the shipping auth methods as: Password, Google/GitHub/Apple/Microsoft OAuth, magic links, passkeys, TOTP, anonymous, phone, and device flow (RFC 8628).
...
| Device flow | `device()` | RFC 8628 device authorization grant for CLI/TV clients |
...
| Imperative inside app code | `auth.context(ctx)` | `authContext.userId` (use when you cannot wrap via `customQuery`/`customMutation`) |
| Raw HTTP (session or API key) | `auth.http.context(ctx, request)` | `authContext.userId` |
| API key HTTP | `auth.http.action(...)` | `ctx.key.userId` |

Per the upstream multi-access guide, every path resolves to the same `userId`:
...
- `https://convex-auth.pages.dev/getting-started/installation/` (mirror)
...
- `https://auth.estifanos.com/ssr/overview/`
- `https://github.com/robelest/convex-auth`
- `https://github.com/robelest/convex-auth/releases`
